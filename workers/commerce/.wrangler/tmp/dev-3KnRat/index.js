...
// src/academy.ts
var USER_ID = /^[A-Za-z0-9_-]{8,160}$/;
var STABLE_ID = /^[a-z0-9][a-z0-9_-]{2,120}$/;
...
__name(assertUserId, "assertUserId");
function assertStableId(value, code = "invalid_contract") {
  if (!STABLE_ID.test(value)) throw contractError(code);
}
__name(assertStableId, "assertStableId");
...
async function getAcademyProduct(db, productKey) {
...
__name(getAcademyProduct, "getAcademyProduct");
async function reserveAcademyPurchase(db, input) {
  ...
  if (reserved.product_key !== input.productKey || reserved.amount !== input.amount || reserved.currency !== input.currency || reserved.provider !== input.provider) throw contractError("checkout_token_conflict");
  return { purchaseId: reserved.purchase_id, duplicate: reserved.purchase_id !== purchaseId };
}
__name(reserveAcademyPurchase, "reserveAcademyPurchase");
async function attachAcademyCheckout(db, purchaseId, providerOrderId) {
...
__name(attachAcademyCheckout, "attachAcademyCheckout");
async function recordAcademyPaid(db, input) {
  assertUserId(input.userId);
  assertStableId(input.productKey, "invalid_product");
  if (input.provider !== "stripe" || input.currency !== "USD" || !Number.isInteger(input.amount) || !Number.isInteger(input.eventCreated)) throw contractError("invalid_contract");
  ...
  if (eventExists) {
    ...
    ).bind(input.provider, input.providerOrderId).first();
  ...
  if (purchase.user_id !== input.userId || purchase.product_key !== input.productKey || purchase.amount !== input.amount || purchase.currency !== input.currency || purchase.provider_payment_id && purchase.provider_payment_id !== input.providerPaymentId) throw contractError("payment_mismatch");
  const now = Math.floor(Date.now() / 1e3);
  const hash = await payloadHash(input);
  const terminalPurchase = ["refunded", "disputed", "revoked"].includes(purchase.status);
  const stale = purchase.last_provider_event_created !== null && (input.eventCreated < purchase.last_provider_event_created || input.eventCreated === purchase.last_provider_event_created && terminalPurchase);
  if (stale) {
    await db.prepare(
...
__name(recordAcademyPaid, "recordAcademyPaid");
async function recordAcademyCheckoutFailure(db, input) {
  if (input.provider !== "stripe" || !input.eventId || !input.providerOrderId || !Number.isInteger(input.eventCreated)) throw contractError("invalid_contract");
  const purchase = await db.prepare(
    `SELECT purchase_id, user_id, product_key, provider, provider_order_id,
            provider_payment_id, amount, currency, status, last_provider_event_created
...
__name(recordAcademyCheckoutFailure, "recordAcademyCheckoutFailure");
async function recordAcademyPaymentState(db, input) {
  const purchase = await db.prepare(
    `SELECT purchase_id, user_id, product_key, provider, provider_order_id,
            provider_payment_id, amount, currency, status, last_provider_event_created
       FROM academy_purchases WHERE provider = ?1 AND provider_payment_id = ?2`
  ).bind(input.provider, input.providerPaymentId).first();
  if (!purchase) return { matched: false };
  const eventExists = await db.prepare(
    `SELECT event_id FROM academy_events WHERE provider = ?1 AND event_id = ?2`
  ).bind(input.provider, input.eventId).first();
...
__name(recordAcademyPaymentState, "recordAcademyPaymentState");
async function getAcademyAccess(db, userId, courseId) {
  assertUserId(userId);
  if (courseId) assertStableId(courseId, "invalid_course");
  const entitlementQuery = courseId ? `SELECT entitlement_id, course_id, grant_type, status, granted_at, updated_at
         FROM academy_entitlements WHERE user_id = ?1 AND course_id = ?2 ORDER BY updated_at DESC` : `SELECT entitlement_id, course_id, grant_type, status, granted_at, updated_at
  ...
  const [entitlements, purchases] = await Promise.all([
    db.prepare(entitlementQuery).bind(...entitlementArgs).all(),
    db.prepare(
...
__name(getAcademyAccess, "getAcademyAccess");
async function putAcademyProgress(db, input) {
  assertUserId(input.userId);
  assertStableId(input.courseId, "invalid_course");
  assertStableId(input.lessonId, "invalid_lesson");
  const now = Math.floor(Date.now() / 1e3);
  if (!Number.isInteger(input.clientUpdatedAt) || input.clientUpdatedAt <= 0 || input.clientUpdatedAt > now + 300) throw contractError("invalid_timestamp");
  await db.prepare(
...
__name(putAcademyProgress, "putAcademyProgress");
async function listAcademyProgress(db, userId) {
  assertUserId(userId);
...
__name(listAcademyProgress, "listAcademyProgress");
async function createAcademyQuestion(db, input) {
  assertUserId(input.userId);
  assertStableId(input.courseId, "invalid_course");
  ...
  if (question.length < 10 || question.length > 2e3) throw contractError("invalid_question");
  const recent = await db.prepare(
    `SELECT COUNT(*) AS count FROM academy_question_reports
     WHERE user_id = ?1 AND created_at > ?2`
  ).bind(input.userId, Math.floor(Date.now() / 1e3) - 3600).first();
...
__name(createAcademyQuestion, "createAcademyQuestion");
async function listOpenAcademyQuestions(db) {
  const rows = await db.prepare(
...
__name(listOpenAcademyQuestions, "listOpenAcademyQuestions");
async function correctAcademyAccess(db, input) {
  assertUserId(input.userId);
  assertStableId(input.courseId, "invalid_course");
  ...
  ).bind(input.userId, input.courseId, grantIdentity).first();
...
__name(correctAcademyAccess, "correctAcademyAccess");
...
__name(contractError2, "contractError");
function requiredString(value, field, max = 200) {
  const normalized = typeof value === "string" ? value.trim() : "";
  if (!normalized || normalized.length > max) {
    throw contractError2("invalid_contract", `Invalid ${field}.`);
  }
  return normalized;
}
...
function normalizeReserveOrder(input) {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw contractError2("invalid_contract", "Invalid order reservation.");
  }
...
function normalizeStripePaidEvent(input) {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw contractError2("invalid_contract", "Invalid payment event.");
  }
  ...
  if (currency !== "USD" || Object.values(amounts).some(
    ...
    throw contractError2("invalid_contract", "Invalid payment amounts.");
  }
  return {
    eventId,
    eventType,
...
function normalizeRecipient(input) {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw contractError2("invalid_recipient", "Shipping details are missing.");
  }
  ...
  if (countryCode !== "US" || !/^[A-Z]{2}$/.test(stateCode)) {
    throw contractError2("invalid_recipient", "Only U.S. shipping is supported.");
  }
  return {
...
function assertPaidEventMatchesOrder(event, order) {
  if (order.id !== event.orderId || order.currency !== event.currency || order.catalog_revision !== event.catalogRevision || order.cart_fingerprint !== event.cartFingerprint || order.merchandise_subtotal_amount !== event.merchandiseSubtotalAmount || order.shipping_amount !== event.shippingAmount || event.totalAmount !== event.merchandiseSubtotalAmount + event.shippingAmount + event.taxAmount) {
    throw contractError2("payment_mismatch", "Stripe payment did not match the order.");
  }
}
__name(assertPaidEventMatchesOrder, "assertPaidEventMatchesOrder");
function centsToDecimal(amount) {
  if (!Number.isInteger(amount) || amount < 0) {
    throw contractError2("invalid_amount", "Invalid currency amount.");
  }
...
async function recordStripePaid(db, event, dataKey, fulfillmentQueued) {
  ...
  const now = Math.floor(Date.now() / 1e3);
  const payloadHash2 = await sha256Hex(
    JSON.stringify({
...
var CONTACT_SENDER = "contact@forms.hob.farm";
var CONTACT_DESTINATIONS = /* @__PURE__ */ new Set(["support@hob.farm", "hey@hob.farm"]);
...
function contactString(body, field, maxLength) {
  const value = body[field];
  if (typeof value !== "string") return null;
  const normalized = value.trim();
  return normalized && normalized.length <= maxLength ? normalized : null;
}
__name(contactString, "contactString");
function isContactEmail(value) {
  return value.length <= 254 && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value);
}
__name(isContactEmail, "isContactEmail");
async function sendContactEmail(request, env) {
  ...
  if (!to || !CONTACT_DESTINATIONS.has(to) || !replyTo || !isContactEmail(replyTo) || !subject || /[\r\n]/.test(subject) || !text || !html) {
    return json({ error: "invalid_contact_message" }, 400);
  }
...
async function handleLedgerRequest(request, env, url) {
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname === "/internal/academy/access") {
      ...
      const courseId = url.searchParams.get("course_id") ?? void 0;
    ...
    if (request.method === "POST" && url.pathname === "/internal/academy/questions") {
      ...
      return json(await createAcademyQuestion(requireLedger(env), {
        userId: String(body.userId ?? ""),
        courseId: String(body.courseId ?? ""),
        lessonId: typeof body.lessonId === "string" ? body.lessonId : void 0,
...
// .wrangler/tmp/bundle-BKFgwD/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-BKFgwD/middleware-loader.entry.ts
