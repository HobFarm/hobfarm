...
Article discovery metadata follows the authoring contract in `docs/article-discovery-authoring.md`. The optional `seoTitle` controls the document and social title while `title` remains the visible H1 and structured-data headline. `description`, `dek`, and `excerpt` use one shared fallback helper. Use `npm run preview:article-search -- {slug}` to inspect the rendered title contract before release.
