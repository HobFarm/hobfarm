...
//
// `/departments/` is retired. This function is the single chokepoint for
// category URLs; never hardcode one at a call site.

/** Categories that are Presents sections. `hobfarm-presents` is the hub itself. */
const presentsSections: Record<string, string> = {
  "hobfarm-presents": "/presents/",
  "magazine-time-machine": "/presents/magazine-time-machine/",
  funnies: "/presents/funnies/",
};

/**
 * Categories that are Workshop programs. The department slug and the program
 * slug differ for two of these, which is why this is an explicit map.
 */
const workshopPrograms: Record<string, string> = {
  "workshop-notes": "/workshop/workshop-notes/",
  "cute-corrupted": "/workshop/cute-and-corrupted/",
  "before-after-eras": "/workshop/before-and-after/",
};

export function departmentPath(slug: string): string {
  const canonical = resolveDepartment(slug) ?? slug;
  return (
    presentsSections[canonical] ??
    workshopPrograms[canonical] ??
    `/articles/${canonical}/`
  );
}

/** True when the category's page is generated by `/articles/[category]`. */
export function isArticleCategory(slug: string): boolean {
  const canonical = resolveDepartment(slug) ?? slug;
  return !(canonical in presentsSections) && !(canonical in workshopPrograms);
}
