...
Consolidated the site search into a single component that indexes every public collection: articles, projects, gallery, grimoire, help, and changelog. Previously the global search FAB only found articles, even though it rendered on every page; a separate article-page-only modal duplicated the work and depended on Alpine.js, which was removed from the dependency list during the March 2026 site audit.

### What changed

- `src/components/global/Search.astro` now collects entries from all six public collections at build time, normalizes them into a single shape (`type`, `title`, `description`, `href`, optional `tags`, `category`, `date`), and runs a Fuse.js index across them. Draft entries are filtered per each collection's draft convention (`draft: true` for articles and grimoire, `status: "draft"` for gallery; projects, help, and changelog have no draft flag and always include all entries).
- The old article-search component was deleted. It had been unimported and its Alpine directives were inert since Alpine was removed from the dependency list.
...
This completes the Alpine.js cleanup loop opened by the March 2026 site audit (Alpine package removed, but the old article-search component was missed) and resolves the highest-leverage item in the May 2026 site audit's Phase 3a.
