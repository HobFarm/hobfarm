---
title: "The Mouse in the Cat Musical"
excerpt: "Footlight Parade put Billy Barty inside a chorus of dancing cats in 1933. Twenty-four years later, he shared Roger Corman's The Undead with Dick Miller."
dek: "How a mouse in a 1933 Warner Bros. musical connects Busby Berkeley's Hollywood, Roger Corman, Dick Miller, Ridley Scott, and Lucasfilm fantasy."
description: "Follow Billy Barty from the dancing cats of Footlight Parade to Dick Miller in The Undead, then onward into Legend and Willow."
author: "d00d"
tags:
  - 3dm
  - Dick Miller
  - Billy Barty
  - Warner Bros.
  - 1930s
  - character actors
  - fantasy
  - Roger Corman
category: cultural-thread
department: hobfarm-presents
format: article
series: 3dm
presentsSeries: 3dm
entryType: feature
heroImage: "/images/3dm/the-mouse-in-the-cat-musical-hero.webp"
socialImage: "/images/3dm/the-mouse-in-the-cat-musical-hero.webp"
heroAlt: "Original vintage-style illustration of dancing cats and a mouse performer following a filmstrip from a 1930s soundstage into a handmade fantasy cave"
canonical: "/presents/3-degrees-of-dick-miller/the-mouse-in-the-cat-musical/"
publishedAt: 2026-07-10
featured: false
status: published
draft: false
connection:
  origin: "Footlight Parade"
  degreeCount: 3
  chain:
    - name: "Footlight Parade"
      kind: film
      year: 1933
      note: "The Warner Bros. musical contains the Sittin' on a Backyard Fence number."
      evidence: "AFI credits Billy Barty as the mouse in the number."
      source: "https://catalog.afi.com/Film/3881-FOOTLIGHT-PARADE"
    - name: "Billy Barty"
      kind: person
      year: "1924–2000"
      note: "The child performer inside the cat number became a durable fantasy and character actor."
      evidence: "AFI records Barty in both Footlight Parade and The Undead."
      source: "https://catalog.afi.com/Film/52420-THE-UNDEAD"
    - name: "The Undead"
      kind: film
      year: 1957
      note: "Roger Corman's fantasy-horror production casts Barty as the Imp."
      evidence: "The same AFI cast record credits Dick Miller as the leper who sells his soul."
      source: "https://catalog.afi.com/Film/52420-THE-UNDEAD"
    - name: "Dick Miller"
      kind: person
      note: "The route reaches Miller through a documented shared production credit."
      evidence: "Barty and Miller both appear in The Undead."
      source: "https://catalog.afi.com/Film/52420-THE-UNDEAD"
contentWarnings: []
rightsNote: "Original HobFarm artwork is used for the feature illustration. Selected frames from Footlight Parade and The Undead are reproduced within the article for film criticism, historical commentary, performer identification, and visual comparison. The frames remain the property of their respective rights holders and are presented in direct relationship to the claims being discussed. External catalog records are linked for production credits and attribution."
sourceNotes:
  - label: "Footlight Parade"
    url: "https://catalog.afi.com/Film/3881-FOOTLIGHT-PARADE"
    note: "AFI record used for the 1933 release, Warner Bros. production, prologue premise, production credits, and Billy Barty's mouse and little-boy roles."
    type: documented-fact
  - label: "The Undead"
    url: "https://catalog.afi.com/Film/52420-THE-UNDEAD"
    note: "AFI record used for Roger Corman's director and producer credits and the shared cast credits for Billy Barty as the Imp and Dick Miller as the leper."
    type: documented-fact
  - label: "Legend"
    url: "https://catalog.afi.com/Film/57395-LEGEND"
    note: "AFI record used for Ridley Scott's director credit and Billy Barty's role as Screwball."
    type: documented-fact
  - label: "Willow casting notice"
    url: "https://www.lucasfilm.com/news/history-in-objects-willow-casting-notice/"
    note: "Lucasfilm history used for the scale of the Nelwyn casting search and Barty's place in the company assembled for the film."
    type: historical-interpretation
  - label: "The High Aldwin"
    url: "https://www.lucasfilm.com/productions/willow-series-chapter-2-the-high-aldwin/"
    note: "Lucasfilm episode guide identifies Billy Barty as the High Aldwin in the 1988 film."
    type: documented-fact
  - label: "Cats timeline"
    url: "https://www.andrewlloydwebber.com/about/timeline"
    note: "The official timeline records the original London production of Cats opening in 1981. The comparison in this article is a visual joke, not an influence claim."
    type: documented-fact
  - label: "Billy Barty: Life and Times"
    url: "https://billybarty.com/life-and-times/"
    note: "Barty's official site gives his birth date as October 25, 1924, making him eight during Footlight Parade's production and release."
    type: documented-fact
  - label: "Federal Theatre Project collection"
    url: "https://www.loc.gov/collections/federal-theatre-project-1935-to-1939/"
    note: "Library of Congress collection for public-domain and rights-cleared context on Depression-era theatrical labor. It is contextual material, not evidence from the Footlight Parade production."
    type: historical-interpretation
---

In 1933, James Cagney's Chester Kent staged a musical about cats. The cats danced on a backyard fence, Ruby Keeler handled the tap work, and a tiny mouse interrupted the feline courtship. Andrew Lloyd Webber's *Cats* would not open in London until 1981.

Nobody stole anything. Hollywood had simply already done it.

The comparison is a visual echo, not an influence claim. Its real value is the mouse. The performer inside that costume was Billy Barty, eight years old when *Footlight Parade* was made. Follow him out of the cat musical and the joke turns into a route through fifty-five years of screen fantasy.

## The route

*Footlight Parade* (1933) → Billy Barty → *The Undead* (1957) → Dick Miller

That is a complete 3DM connection. The later branch keeps going:

Billy Barty → *Legend* (1985) → *Willow* (1988)

The route starts with a studio musical, passes through Roger Corman's bargain-scale fantasy-horror world, and ends inside the practical-fantasy boom of the 1980s. One working actor carries the line.

## Everybody was broke

*Footlight Parade* turns workplace panic into a musical factory. Chester Kent begins as a Broadway producer knocked out of his old business by talking pictures. He adapts by building short live "prologues" that theaters can stage before the feature.

The [AFI Catalog](https://catalog.afi.com/Film/3881-FOOTLIGHT-PARADE) describes Kent and his partners taking on more prologue work than they can comfortably handle while competitors steal ideas and business partners work their own angles. [Turner Classic Movies](https://www.tcm.com/articles/12783/footlight-parade-1933) places the story directly inside the Great Depression and the arrival of sound.

The movie's backstage comedy runs on exhaustion. Ideas become inventory. Performers move between rehearsal rooms and stages while Kent tries to feed an entertainment pipeline that cannot stop. The gigantic numbers grow out of that pressure. Spectacle is the product at the far end of an anxious workplace.

<figure class="article-wide">
  <img src="https://cdn.hob.farm/articles/3dm-mouse-cat-musical/exhausted-chester-kent.png" alt="James Cagney as Chester Kent reclines exhausted in an office while coworkers continue working around him in Footlight Parade." width="1424" height="1072" loading="lazy" decoding="async">
  <figcaption>Chester Kent, played by James Cagney, runs out of steam inside the prologue factory.</figcaption>
</figure>

The wider studio system is mapped in [1933: The Year Warner Bros. Built a World](/presents/3-degrees-of-dick-miller/1933-the-year-warner-bros-built-a-world/).

## Chester Kent invents *Cats*

"Sittin' on a Backyard Fence" is a miniature cat musical. Performers wear feline costumes. Courtship becomes choreography. Ruby Keeler takes the center while the ensemble turns an alley fence into a stage.

<figure class="article-wide">
  <img src="https://cdn.hob.farm/articles/3dm-mouse-cat-musical/cats-backstage.png" alt="The Sittin' on a Backyard Fence musical number performed on an oversized city alley set while musicians and observers watch." width="1424" height="1072" loading="lazy" decoding="async">
  <figcaption>The cat number seen from behind the orchestra, with the oversized alley set and feline chorus filling the stage.</figcaption>
</figure>

<div class="article-grid cols-3" role="group" aria-label="Cat costume gallery from Footlight Parade">
  <figure>
    <img src="https://cdn.hob.farm/articles/3dm-mouse-cat-musical/cats-performers.png" alt="A large chorus of women in matching cat costumes performs on multiple levels of a theatrical alley set." width="1424" height="1072" loading="lazy" decoding="async">
    <figcaption>The chorus forms a vertical wall of cats across the alley scenery.</figcaption>
  </figure>
  <figure>
    <img src="https://cdn.hob.farm/articles/3dm-mouse-cat-musical/cats-ruby-close-up.png" alt="Ruby Keeler performs in a light-colored cat costume with other cat-costumed dancers behind her." width="1424" height="1072" loading="lazy" decoding="async">
    <figcaption>Ruby Keeler at the center of the feline chorus.</figcaption>
  </figure>
  <figure>
    <img src="https://cdn.hob.farm/articles/3dm-mouse-cat-musical/cats-sittin.png" alt="Ruby Keeler and Dick Powell sit together in contrasting cat costumes during Sittin' on a Backyard Fence." width="1424" height="1072" loading="lazy" decoding="async">
    <figcaption>Ruby Keeler and Dick Powell play the number's central pair of courting cats.</figcaption>
  </figure>
</div>

Then the mouse arrives.

<figure class="article-wide">
  <img src="https://cdn.hob.farm/articles/3dm-mouse-cat-musical/mouse-wide.png" alt="A small mouse-costumed performer crosses the floor while dancers dressed as cats watch in Footlight Parade." width="1424" height="1072" loading="lazy" decoding="async">
  <figcaption>The scale of the joke becomes visible when the mouse enters the world of the adult cat chorus.</figcaption>
</figure>

Billy Barty scampers into the number as a disruptive scale change, a child-sized performer puncturing the elegance of the adult chorus. AFI credits him as the "Mouse in 'Sittin' on a Backyard Fence'" and also as the little boy in "Honeymoon Hotel." The same record credits Larry Ceballos with dance direction on those two numbers while Busby Berkeley handled the movie's larger musical design.

<figure class="article-wide">
  <img src="https://cdn.hob.farm/articles/3dm-mouse-cat-musical/mouse-close-up.png" alt="Child performer Billy Barty in a dark mouse costume beside an oversized mousetrap and giant milk bottles." width="1424" height="1072" loading="lazy" decoding="async">
  <figcaption>Billy Barty as the mouse, surrounded by the number's oversized scenery and props.</figcaption>
</figure>

To a modern viewer, the sequence looks uncannily like an ancestor of *Cats*: costumed felines, romantic types, ensemble movement, and a whole social order explained through dance. That resemblance is the joke. The documented connection begins when we stop looking at the cats and identify the mouse.

## Follow the mouse

Barty did not stay trapped inside a childhood novelty act. His credits kept moving.

In *Footlight Parade*, he is inside Warner Bros.' early-sound musical machine. Twenty-four years later, he appears in *The Undead*, a Roger Corman production made on an entirely different scale. AFI credits Barty as the Imp and Dick Miller as the leper who sells his soul. They are not connected by genre atmosphere, studio ancestry, or a clever theory. They worked on the same picture.

That shared credit is the hinge.

## The route reaches Dick Miller

Barty and Miller do not appear together. Miller's brief scene as the leper is followed by Barty's appearance as the Imp. The connection is a shared production credit, not an on-screen meeting.

<div class="article-grid undead-comparison" role="group" aria-label="Separate appearances by Dick Miller and Billy Barty in The Undead">
  <figure>
    <img src="https://cdn.hob.farm/articles/3dm-mouse-cat-musical/dick-miller-undead-1957.jpg" alt="Close-up of Dick Miller in ragged medieval clothing as the leper in Roger Corman's The Undead." width="1015" height="766" loading="lazy" decoding="async">
    <figcaption>Dick Miller appears briefly as the leper in <em>The Undead</em>.</figcaption>
  </figure>
  <figure>
    <img src="https://cdn.hob.farm/articles/3dm-mouse-cat-musical/billy-barty-undead-1957.jpg" alt="Billy Barty as the Imp sits beside a woman in a medieval fantasy interior in The Undead." width="1007" height="758" loading="lazy" decoding="async">
    <figcaption>Billy Barty appears in the following sequence as the Imp.</figcaption>
  </figure>
</div>
<p class="article-grid-caption">They never occupy the same frame. Their nearby appearances and shared cast credit provide the documented 3DM connection.</p>

*The Undead* has a hypnotist, a time-travel experiment, medieval France, witches, Satan, an Imp, and Dick Miller selling his soul as a leper. Roger Corman directed and produced it. Mel Welles and Bruno VeSota appear nearby, two more familiar faces in Corman's production network.

The contrast with *Footlight Parade* is almost too clean. Warner Bros. gave Barty a musical alley filled with dancing cats. Corman gave him fog, a medieval costume, and a handmade fantasy world assembled under much tighter limits.

One performer physically connects those production systems. The route exposes something a genre list cannot. Studio spectacle and low-budget fantasy were not sealed historical compartments. Working actors carried experience, habits, and recognizable screen types between them.

That is the 3DM principle. Start with a visible oddity. Identify the worker inside it. Follow the credits until two supposedly separate movie worlds touch.

## The fantasy branch

Barty's career keeps the route open after it reaches Dick Miller.

The [AFI Catalog record for *Legend*](https://catalog.afi.com/Film/57395-LEGEND) credits him as Screwball in Ridley Scott's dark fairy tale. Three years later, he played the High Aldwin in *Willow*. Lucasfilm's own retrospective identifies Barty in the role and describes the much larger company of short actors assembled for the Nelwyn community.

The mouse becomes the Imp. The Imp becomes Screwball. Screwball becomes the High Aldwin.

This is not a line of influence. It is a line of labor. Barty moved through radically different ways of manufacturing fantasy:

- Warner Bros. musical spectacle in the early sound era.
- Corman's compressed fantasy-horror production system.
- Ridley Scott's enormous, heavily designed dark fairy tale.
- George Lucas and Ron Howard's heroic fantasy world.

The costumes, budgets, effects, and audiences changed. The specialized performer remained useful.

## What the connection reveals

Film history usually puts the largest name at the center. The better connections often belong to people working at the edge of the poster.

Barty's career joins the pre-Code musical to the Corman repertory system, then carries that line into creature performance and the practical-effects fantasy boom. He was present when fantasy meant a painted studio alley, when it meant a fog machine and a cave, and when it meant enormous location companies and elaborate physical worlds.

That continuity is the story. The route begins as a joke about dancing cats and ends as a compact history of how Hollywood reused specialized performers across six decades.

Start with the cats. Follow the mouse. Eventually, you reach Dick Miller.

## The video route

A four-to-six-minute video version is in development for the 3DM Film Room. It will keep the same evidence chain visible on screen and separate documented credits from the *Cats* comparison. Archival context will come from item-level rights records, including material in the Library of Congress Federal Theatre Project collection. Copyrighted film frames will only be used when the editorial purpose and rights basis are clear.
