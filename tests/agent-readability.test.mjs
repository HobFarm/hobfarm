...
test("agent-readable corpus routes and markdown alternates are wired", () => {
  ...
  for (const route of [
    ...
    assert.match(corpus, new RegExp(route.replaceAll("/", "\\/").replaceAll(".", "\\.")));
  }
});
...
test("llms corpus excludes private, raw, paid, and thin discovery surfaces", () => {
  ...
  for (const forbidden of [
    ...
    assert.match(
      corpus,
      new RegExp(`forbiddenAgentPattern[\\s\\S]*${forbidden.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`),
...
test("global headers advertise discovery and deliberate content signals", () => {
  ...
  assert.match(headers, /Link:\s*<\/sitemap\.xml>; rel="sitemap"; type="application\/xml"/);
  assert.match(headers, /Link:\s*<\/llms\.txt>; rel="alternate"; type="text\/plain"/);
  assert.match(
    headers,
    /Link:\s*<\/\.well-known\/agent-skills\/index\.json>; rel="service-desc"; type="application\/json"/,
  );
  assert.match(headers, /Content-Signal:\s*ai-train=no, search=yes, ai-input=yes/);
...
test("markdown negotiation middleware serves markdown only for public content routes", () => {
  ...
  assert.match(middleware, /Content-Signal": "ai-train=no, search=yes, ai-input=yes"/);
...
test("agent skill index is valid and matches the skill digest", () => {
  const manifest = JSON.parse(read("public/.well-known/agent-skills/index.json"));
  const skill = read("public/.well-known/agent-skills/read-hobfarm-corpus/SKILL.md");
  ...
  assert.equal(manifest.skills[0].url, "/.well-known/agent-skills/read-hobfarm-corpus/SKILL.md");
...
test("no fake protocol discovery catalogs are added", () => {
  for (const file of [
    ...
    assert.equal(exists(file), false, `${file} should not exist without a real implementation`);
  }
});
...
test("agent-readable public surfaces expose appropriate structured data", () => {
  ...
  const styleFusion = read("src/components/projects/StyleFusionProjectPage.astro");
