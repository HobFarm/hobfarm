...
Give images, captions, alt text, source links, related reading, social metadata, and structured data the treatment supported by the current article form. Follow the static scheduling contract in `docs/codex/publishing-surfaces.md`: a publication timestamp sets release eligibility, not an executing scheduler. Commit and push an approved scheduled article before its release time when the user authorizes that release. The permanent scheduled-release monitor then checks the live route, triggers the post-timestamp Pages build when needed, and verifies publication. Do not create an article-specific workflow or a second deployment path.
