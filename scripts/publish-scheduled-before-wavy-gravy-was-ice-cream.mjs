...
if (!publicationMatch) {
  throw new Error("Before Wavy Gravy Was Ice Cream has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("Before Wavy Gravy Was Ice Cream is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("Before Wavy Gravy Was Ice Cream is neither scheduled nor published.");
}

await writeFile(articlePath, article.replace(/^status:\s*scheduled$/m, "status: published"), "utf8");
console.log("Marked Before Wavy Gravy Was Ice Cream as published.");
await setOutput("true");
