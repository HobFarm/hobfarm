...
const articlePath = resolve("src/content/articles/i-want-my-mtv.mdx");
...
if (!publicationMatch) {
  throw new Error("I Want My MTV has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("I Want My MTV is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("I Want My MTV is neither scheduled nor published.");
}

await writeFile(articlePath, article.replace(/^status:\s*scheduled$/m, "status: published"), "utf8");
console.log("Marked I Want My MTV as published.");
await setOutput("true");
