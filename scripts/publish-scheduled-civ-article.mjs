...
const articlePath = resolve("src/content/articles/i-could-be-playing-civilization.md");
const expectedPublication = "2026-07-28T17:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("The Civilization article has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("The Civilization article is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("The Civilization article is neither scheduled nor published.");
}
...
console.log("Marked the Civilization article as published.");
await setOutput("true");
