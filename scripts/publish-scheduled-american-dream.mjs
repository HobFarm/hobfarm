...
const expectedPublication = "2026-08-05T16:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("Fear and Loathing After the American Dream has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("Fear and Loathing After the American Dream is already published.");
...
if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error(
    "Fear and Loathing After the American Dream is neither scheduled nor published.",
  );
}
...
console.log("Marked Fear and Loathing After the American Dream as published.");
await setOutput("true");
