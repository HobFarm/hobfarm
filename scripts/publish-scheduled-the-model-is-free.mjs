...
const articlePath = resolve("src/content/articles/the-model-is-free.mdx");
const expectedPublication = "2026-08-13T16:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("The Model Is Free has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("The Model Is Free is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("The Model Is Free is neither scheduled nor published.");
}
...
console.log("Marked The Model Is Free as published.");
await setOutput("true");
