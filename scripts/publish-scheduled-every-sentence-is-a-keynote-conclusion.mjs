...
const articlePath = resolve("src/content/articles/every-sentence-is-a-keynote-conclusion.mdx");
const expectedPublication = "2026-08-15T16:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("Every Sentence Is a Keynote Conclusion has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("Every Sentence Is a Keynote Conclusion is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("Every Sentence Is a Keynote Conclusion is neither scheduled nor published.");
}

await writeFile(articlePath, article.replace(/^status:\s*scheduled$/m, "status: published"), "utf8");
console.log("Marked Every Sentence Is a Keynote Conclusion as published.");
await setOutput("true");
