...
const articlePath = resolve("src/content/articles/the-number-one-song-ive-never-heard.mdx");
const expectedPublication = "2026-08-13T16:19:00-07:00";
...
if (!publicationMatch) {
  throw new Error("The Number One Song I've Never Heard has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("The Number One Song I've Never Heard is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("The Number One Song I've Never Heard is neither scheduled nor published.");
}
...
console.log("Marked The Number One Song I've Never Heard as published.");
await setOutput("true");
