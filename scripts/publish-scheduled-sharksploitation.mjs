...
const articlePath = resolve("src/content/articles/sharksploitation.mdx");
const expectedPublication = "2026-08-07T16:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("Sharksploitation has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("Sharksploitation is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("Sharksploitation is neither scheduled nor published.");
}
...
console.log("Marked Sharksploitation as published.");
await setOutput("true");
