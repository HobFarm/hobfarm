...
const articlePath = resolve("src/content/articles/too-big-for-the-box.md");
const expectedPublication = "2026-07-29T17:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("Too Big for the Box has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("Too Big for the Box is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("Too Big for the Box is neither scheduled nor published.");
}
...
console.log("Marked Too Big for the Box as published.");
await setOutput("true");
