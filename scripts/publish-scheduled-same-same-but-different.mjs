...
const articlePath = resolve("src/content/articles/same-same-but-different.mdx");
const expectedPublication = "2026-08-12T16:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("Same Same, But Different has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("Same Same, But Different is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("Same Same, But Different is neither scheduled nor published.");
}
...
console.log("Marked Same Same, But Different as published.");
await setOutput("true");
