...
const articlePath = resolve("src/content/articles/hit-the-source-directly.mdx");
const expectedPublication = "2026-08-21T16:20:00-07:00";
...
if (!publicationMatch) throw new Error("Hit the Source Directly has no publishedAt field.");
...
if (/^status:\s*published$/m.test(article)) {
  console.log("Hit the Source Directly is already published.");
  await setOutput("true");
  process.exit(0);
}
if (!/^status:\s*scheduled$/m.test(article)) throw new Error("Hit the Source Directly is neither scheduled nor published.");
await writeFile(articlePath, article.replace(/^status:\s*scheduled$/m, "status: published"), "utf8");
console.log("Marked Hit the Source Directly as published.");
await setOutput("true");
