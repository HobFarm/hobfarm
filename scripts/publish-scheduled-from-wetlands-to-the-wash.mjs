...
const articlePath = resolve("src/content/articles/from-wetlands-to-the-wash.mdx");
const expectedPublication = "2026-08-17T16:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("From Wetlands to the Wash has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("From Wetlands to the Wash is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("From Wetlands to the Wash is neither scheduled nor published.");
}

await writeFile(articlePath, article.replace(/^status:\s*scheduled$/m, "status: published"), "utf8");
console.log("Marked From Wetlands to the Wash as published.");
await setOutput("true");
