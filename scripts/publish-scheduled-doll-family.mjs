...
const articlePath = resolve("src/content/articles/they-had-names-doll-family.mdx");
const expectedPublication = "2026-07-26T23:40:00-07:00";
...
if (!publicationMatch) {
  throw new Error("The Doll Family article has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("The Doll Family article is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("The Doll Family article is neither scheduled nor published.");
}
...
console.log("Marked the Doll Family article as published.");
await setOutput("true");
