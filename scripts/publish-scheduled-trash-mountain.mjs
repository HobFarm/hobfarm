...
const articlePath = resolve("src/content/articles/trash-mountain.mdx");
const expectedPublication = "2026-08-11T16:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("Trash Mountain has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("Trash Mountain is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("Trash Mountain is neither scheduled nor published.");
}
...
console.log("Marked Trash Mountain as published.");
await setOutput("true");
