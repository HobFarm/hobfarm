...
const expectedPublication = "2026-08-09T16:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("Mr. Paige has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("Mr. Paige is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("Mr. Paige is neither scheduled nor published.");
}
...
console.log("Marked Mr. Paige as published.");
await setOutput("true");
