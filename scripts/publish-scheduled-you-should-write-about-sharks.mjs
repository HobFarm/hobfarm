...
const articlePath = resolve(
  "src/content/articles/you-should-write-about-sharks.mdx",
);
const expectedPublication = "2026-08-08T16:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("You Should Write About Sharks has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("You Should Write About Sharks is already published.");
...
if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error(
    "You Should Write About Sharks is neither scheduled nor published.",
  );
}
...
console.log("Marked You Should Write About Sharks as published.");
await setOutput("true");
