...
const articlePath = resolve("src/content/articles/put-on-the-glasses.md");
const expectedPublication = "2026-07-30T17:20:00-07:00";
...
if (!publicationMatch) {
  throw new Error("Put On the Glasses has no publishedAt field.");
}
...
if (/^status:\s*published$/m.test(article)) {
  console.log("Put On the Glasses is already published.");
  await setOutput("true");
  process.exit(0);
}

if (!/^status:\s*scheduled$/m.test(article)) {
  throw new Error("Put On the Glasses is neither scheduled nor published.");
}
...
console.log("Marked Put On the Glasses as published.");
await setOutput("true");
