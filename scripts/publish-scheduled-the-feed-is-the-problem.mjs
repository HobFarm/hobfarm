...
const articlePath = resolve("src/content/articles/the-feed-is-the-problem.mdx");
const expectedPublication = "2026-08-22T16:20:00-07:00";
...
if (!publicationMatch) throw new Error("The Feed Is the Problem has no publishedAt field.");
...
if (/^status:\s*published$/m.test(article)) {
  console.log("The Feed Is the Problem is already published.");
  await setOutput("true");
  process.exit(0);
}
if (!/^status:\s*scheduled$/m.test(article)) throw new Error("The Feed Is the Problem is neither scheduled nor published.");
await writeFile(articlePath, article.replace(/^status:\s*scheduled$/m, "status: published"), "utf8");
console.log("Marked The Feed Is the Problem as published.");
await setOutput("true");
